#!/usr/bin/env bash
# Set MONGO_URI on a Render web service and trigger a deploy.
# Usage:
...
PAYLOAD=$(MONGO_URI="$URI" python3 -c 'import json,os; print(json.dumps({"key":"MONGO_URI","value":os.environ["MONGO_URI"]}))')
